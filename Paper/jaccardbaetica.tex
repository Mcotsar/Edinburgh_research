...
%Did each province follow a different pattern for its distribution? 
%\xavi{no entiendo esta pregunta} what patterns were followed for the distribution of olive oil to the different provinces? 
%\xavi{no es esta pregunta idéntica a la anterior?} 
%\xavi{esta frase no tiene sentido aquí; o la pones antes o en otro párrafo, pero es raro acabar un set de 2 preguntas con una frase que no sea otra pregunta} 
...
The application of different quantitative approaches has allowed us to improve our interpretation of the growing amount of archaeological evidence and reveal the dynamics of the Roman production %\xavi{en qué sentido? qué quieres decir aquí con complexity? Si no especificas queda muy genérico (rollo "el mundo es muy complejo")}.
...
%\xavi{qué quiere decir que la cronología es diversa?} 
...
%were shipped and distributed from \textit{Baetica} throughout western Europe. 
...
%The usage period is associated with a large amphora production for almost three centuries\xavi{esto está repetido no?}. 
...
%\xavi{qué information? El código de letras?}. 
...
%Additionally, a large percentage of Dressel 20 containers were not stamped thus complicating the interpretation~\xavi{esta última frase aquí es un poco random no?}. 
...
%\xavi{no entiendo esto...allowed o required?}. 
...
Some hypotheses argue elsewhere that this same \textit{Annona} could also have organized the supply of additional goods such as olive oil to the Roman legions \citep{remesal_annona_1986,
remesal_annona_1990}
...
%such as \textit{Germania} (Remesal Rodrıguez, 1986)\xavi{por qué such as Germania? Aún no has explicado qué hacen en Germania}. 
...
Roman studies exploring the presence of Dressel 20 amphorae in the provinces Inferior and Superior have not had the same impact as the rest of the provinces.
%\xavi{no entiendo esta frase. Que qué provincia? Germania son 2 provincias no?}

This could be explained due to the lack of archaeological sources \citep{horacio2010llegada}. As a consequence, it is still unknown if trade agents participated on the distribution of Baetican olive oil in \textit{Germania} or the supply was exclusively focused on the Roman army garrisons\citep[156]{remesal_germn_2010}\xavi{quieres decir que aquí sólo encontramos dressel 20 en yacimientos militares porque no se han excavado muchos civiles?}. 
...
%To do that, we use the CEIPAC database to collect stamps from different places. The CEIPAC dataset contains over 50.000 of epigraphy records found in amphorae, mostly from \textit{Monte Testaccio}.
...
%\xavi{estaría bien ampliar un poco esto; qué organizaban exactamente?}
...
%\xavi{el gráfico es dificil de entender...yo leo que hay unos 20 workshops con 1 solo stamp, por lo que entiendo que no es cierto que la mayoría solo tengan 1 sello (hay como 50 workshops con entre 2 y 30 sellos no?)} 
...
%\xavi{una cosa, aquí has pasado de sellos a códigos, y los valores son distintos que en el gráfico anterior. Deberías clarificarlo antes de empezar a hablar de la figura}. 
...
%This method allowed us to bear in mind a similar number of times for each repeated stamp per workshop\xavi{no entiendo esta frase}. 
...
%even if most sites mostly showed different stamps~\xavi{quieres decir que los sitios más cercanos al limes son más similares? no queda claro y hay que detallarlo un poco más}.
...
%The general purpose was to explore if such differences found in the stamp codes could play an important role in the Roman market\xavi{este párrafo es un poco reiterativo no?}.  
...
%In general, the majority of stamps were located in different amphora workshops\xavi{estas 2 frases repiten cosas dichas antes}. 
...
%These stamp codes detected in closer workshops do not move from other distant workshops. In other words, the stamps tend to remain in the same area and different stamps were located in a same amphora workshop\xavi{de nuevo estas 2 frases repiten cosas de antes}. 
...
%\xavi{estos 2 párrafos igual deberían ir a la discusión no? La conclusión debería ser algo más general y aquí estás aún interpretando resultados}.  
...
%\xavi{y esto para qué serviría? debería explicarse un poco más}.
...
%Rather, it would imply that olive oil production was distributed from non-specific production centres.
